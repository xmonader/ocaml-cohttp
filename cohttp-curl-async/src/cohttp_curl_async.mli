...
module Context : sig
  (** A context shares the curl event handling logic for all all curl requests
      associated to it *)
...
module Request : sig
  ...
  type 'a t
  (** ['a t] represents an http request ['a] determines who the response body is
      handled. *)

  val create :
    ?timeout:Core_kernel.Time.Span.t (** timeout for the request *) ->
